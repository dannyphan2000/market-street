...
function isValidResponse(response) {
    return response && Array.isArray(response.deliveryEstimates);
}
...
exports.estimate = function (result) {
    try {
        ...
        for (var i = 0; i < groups.length; i++) {
            ...
            if (!requestBody) {
                logger.debug('CDS estimate hook skipped group {0}; no product, destination, or CDS-mapped shipping methods.', i);
                continue;
            }
            ...
            if (!isValidResponse(response)) {
                ...
                if (!serviceResult || !serviceResult.ok || !isValidResponse(serviceResult.object)) {
                    logger.error('CDS delivery estimate unavailable or invalid for group {0}.', i);
                    return new Status(
                        Status.ERROR,
                        'CDS_ESTIMATE_UNAVAILABLE',
                        'CDS Shipping delivery estimate call failed or returned an invalid response.'
                    );
                }
            ...
            if (logger.isDebugEnabled()) logger.debug('CDS delivery estimate response for group {0}: {1}', i, JSON.stringify(response));
            var groupPopulated = responseMapper.mapToResult(response, groups[i]);
            logger.debug('CDS estimate hook populated {0} shipping option(s) for group {1}.', groupPopulated, i);
            populated += groupPopulated;
        }

        logger.debug('CDS estimate hook populated {0} shipping option(s).', populated);
        return new Status(Status.OK);
    } catch (e) {
