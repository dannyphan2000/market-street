...
function getTokenEndpointUrl(baseUrl, deliveryConfig) {
    return baseUrl.replace(/\/$/, '') +
        '/dwsso/oauth2/access_token?grant_type=client_credentials&scope=' +
...
function createService(deliveryConfig) {
    return LocalServiceRegistry.createService(SERVICE_ID, {
        ...
        filterLogMessage: function (msg) {
            // Redact the Basic-auth client secret (request header) and the OAuth
            // access token (response body) from the service communication log.
            return String(msg || '')
